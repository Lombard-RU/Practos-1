...
imena = {
    "hero" : "Герой",
    "Старик": "(Седобородый старик)"
}

print(
    Fore.GREEN + "*Вы очутились посередине поляны в лесу. Перед вами стоит седобородый старик*\n"
    + Fore.CYAN + imena["Старик"] + ": Приветствую тебя, странник ты не просто так сюда забрёл, тебя запутал Леший! \n"
    + Fore.YELLOW + imena["hero"] + ": Что же мне делать? \n"
    + Fore.CYAN + imena["Старик"] + ": Найди перо жарптицы, кулон с коловратом, и щит с руной Алатырь."
    + Style.RESET_ALL
)
klichka = ""
enemies_lvl_1 = ["Гоблин", "Злой гном"]
inventory = []
heal = 10
proshol_1 = False
proshol_2 = False
...
def pobeda():
    if proshol_3 and proshol_2 and proshol_1 == True:
            print(
                Fore.CYAN + "Вы возвращаетесь на поляну и отдаёте все собранные артефакты персонажу: " + imena["Старик"]+ " и он выводит вас из леса\n"
                + Fore.GREEN
                + "🎉Вы победили🎉"
                + Style.RESET_ALL
            )
...
def igra():
    ...
    if result == "Вы пошли на юг":
        ...
        if f == 1:
            print(Fore.RED + " Вы обожглись пером, вы потеряли 2 здоровья ❤️" + Style.RESET_ALL)
            heal -= 2
            dead()
    ...
    if result == "Вы пошли на север":
        ...
        if sever != 3:
            heal -= 2
            print(Fore.RED + "*Ловушка срабатывает и вы теряете 2 здоровья ❤️*" + Style.RESET_ALL)
            dead()
        print(
            Fore.GREEN + "*Вы проходите и оказываетесь перед пнём на котором лежит щит с руной Алатыр🛡️. Вы берёте его и возвращаетесь на поляну.*" + Style.RESET_ALL
        )
    ...
    if result == "Вы пошли на восток":
        ...
        if vostok not in ogni:
            ...
            dead()
        print(
            Fore.GREEN + "*Стена отодвигается и вы берёте кулон с коловратом☀️, который лежал на полу пещеры за стеной*" + Style.RESET_ALL
        )
        proshol_3 = True
        pobeda()
        vopr()
